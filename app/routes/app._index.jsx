...
export const loader = async ({ request }) => {
  ...
  /*
   * Do NOT automatically create fake/default campaigns.
   * The dashboard should reflect the merchant's real rules.
   */
...
export const action = async ({ request }) => {
  ...
  if (intent === "DUPLICATE") {
    ...
    const duplicate = await db.badge.create({
      data: {
        shop,
    ...
    /*
     * Duplicate attached products as well.
     */
    const existingProducts = await db.badgeProduct.findMany({
      where: {
        badgeId: existing.id,
      },
    });
    ...
    if (existingProducts.length > 0) {
      await db.badgeProduct.createMany({
  ...
  if (intent === "SAVE_SETTINGS") {
    const globalCustomCss =
      String(formData.get("globalCustomCss") || "");
  ...
  const name =
    ...
    "Untitled Badge Campaign";

  const enabled =
    formData.get("enabled") === "true";

  const text =
    String(formData.get("text") || "BADGE").trim();

  const icon =
    String(formData.get("icon") || "");

  const bgColor =
    String(formData.get("bgColor") || "#111827");

  const textColor =
    String(formData.get("textColor") || "#FFFFFF");

  const borderColor =
    String(formData.get("borderColor") || "#000000");

  const position =
    String(formData.get("position") || "TOP_LEFT");

  const shape =
    String(formData.get("shape") || "PILL");

  const fontSize =
    parseInt(formData.get("fontSize") || "12", 10);

  const fontWeight =
    String(formData.get("fontWeight") || "bold");

  const paddingX =
    parseInt(formData.get("paddingX") || "10", 10);

  const paddingY =
    parseInt(formData.get("paddingY") || "4", 10);

  const borderRadius =
    parseInt(formData.get("borderRadius") || "20", 10);

  const priority =
    parseInt(formData.get("priority") || "0", 10);

  const targetType =
    String(formData.get("targetType") || "GLOBAL");

  const targetTags =
    String(formData.get("targetTags") || "");

  const minInventory =
    parseInt(formData.get("minInventory") || "0", 10);

  const maxInventory =
    parseInt(formData.get("maxInventory") || "9999", 10);

  const minPrice =
    parseFloat(formData.get("minPrice") || "0");

  const maxPrice =
    parseFloat(formData.get("maxPrice") || "99999");

  const customCss =
    String(formData.get("customCss") || "");

  const hideOnMobile =
    formData.get("hideOnMobile") === "true";

  const hideOnDesktop =
    formData.get("hideOnDesktop") === "true";
  ...
  await db.$transaction(async (tx) => {
    ...
    /*
     * Replace selected product relationships.
     */
...
export default function SaaSAdminApp() {
  ...
  const [globalCssState, setGlobalCssState] =
    useState(settings?.globalCustomCss || "");
  ...
  const handleToggleBadge = (badge) => {
    ...
    Object.entries(badgeToForm(badge)).forEach(
      ([key, value]) => {
        if (key === "productIds") {
          data.append(
            key,
  ...
  const handleResourcePicker = async () => {
    if (
      ...
      window.alert(
        "Shopify Resource Picker is not available in this session."
      );
      return;
    }
  ...
  const previewBadgeStyle = {
    ...
    border: `1px solid ${
      formData.borderColor || formData.bgColor
    }`,
            ...
            {selectedTab === 0 && (
              <BlockStack gap="400">
                ...
                >
                  <BlockStack gap="100">
                    ...
                    <Text
                      ...
                      tone="subdued"
                    >
                      Manage every badge displayed on your
                      storefront.
                    </Text>
                  </BlockStack>
                ...
                {badges.length === 0 ? (
                  ...
                  >
                    <p>
                      Create a product badge and publish
                      it to your Shopify storefront.
                    </p>
                  </EmptyState>
                ) : (
                  <IndexTable
              ...
              <BlockStack gap="500">
                ...
                <BlockStack gap="100">
                  ...
                  <Text
                    ...
                    tone="subdued"
                  >
                    Performance collected from your
                    storefront badge events.
                  </Text>
                </BlockStack>
              ...
              <BlockStack gap="500">
                ...
                <BlockStack gap="100">
                  ...
                  <Text
                    ...
                    tone="subdued"
                  >
                    Configure global styling applied to
                    storefront badges.
                  </Text>
                </BlockStack>
                ...
                <Card padding="500">
                  <BlockStack gap="400">
                    ...
                    <Text
                      ...
                      tone="subdued"
                    >
                      Optional CSS that will be loaded by
                    ...
                    >
                      <BlockStack gap="100">
                        ...
                        <Text
                          ...
                          tone="subdued"
                        >
                          Control whether this badge
                          can appear on your storefront.
                        </Text>
                      </BlockStack>
